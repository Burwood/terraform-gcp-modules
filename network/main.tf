...
resource "google_compute_subnetwork" "subnetwork" {
  count = "${!var.auto_create_subnetworks && length(var.subnets) > 0 && var.create_vpc_secondary_ranges == 1 ? length(var.subnets):0 }"
...
resource "google_compute_subnetwork" "subnetworknoalias" {
  count = "${!var.auto_create_subnetworks && length(var.subnets) > 0 && var.create_vpc_secondary_ranges == 0 ? length(var.subnets):0 }"
...
resource "google_compute_route" "route" {
  count   = "${!var.auto_create_subnetworks && length(var.routes) > 0 ? length(var.routes) : 0}"
  project = "${var.project_id}"
